...
RUN_TAG     = "run31_DP_400demos_obs4"
PATCH_TYPE  = "red"
SAVE_DIR    = "../eval_results_DP_obs4_clean_rollout"
...
def predict_actions_batched(model, observations, device, obs_horizon=4, subsample=10):
    all_preds = []
    for i in range(0, len(observations), subsample):
        start  = max(0, i - obs_horizon + 1)
        window = list(observations[start:i+1])
        if len(window) < obs_horizon:                       # near the start — repeat earliest frame
            window = [window[0]] * (obs_horizon - len(window)) + window
        p, _ = model.predict(window, device=device)
...
acc_mean = []
acc_std  = []
model    = DiffusionPolicyNetwork(obs_horizon=4).to(device)
...
for P in P_LEVELS:
    ...
    seed_accs             = []
    SUBSAMPLE = 10
    actions_eval = actions[::SUBSAMPLE]   # must match predict_actions_batched's subsample stride

    gt_is_target          = is_target_action(actions_eval)
    ...
    for mseed in tqdm(MODEL_SEEDS, desc=f"P={P} | model seeds"):
        ...
        preds          = predict_actions_batched(model, observations, device=device, obs_horizon=4, subsample=10)
